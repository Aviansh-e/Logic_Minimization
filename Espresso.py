def expandIn(arr, off, cont, n, lee):
    ...
    # here arr2 will be that array which will go for coverd by expansion or not
    def meancover(arr, arr2):
            ...
            if(dup[i] == dot[j] and (j not in lis)):
                ...
                for l in range(len(dot)):
                   ...
                   # print(arr[l],c,lan,dup,dot)
                    # if c=1 tells us that arr[i] get covered by the arr[k] so we shouldn't need send it for the expansion
                    if(c == 1 and dot[l] in dup):
...
def inputConv(n, str, str2):
    ...
    for i in range(lee):
        col = []
        # it run in variable to variable in a particular Minterm
        for j in range(i*(n+1), i*(n+1)+n):
            if(str[j] in str1):
                col.append(0)
                col.append(1)
            else:
...
def printOut(n, str, str2):
    ...
    # here we are storing the list of expansion  in new list
    new = expandIn(arr, off, cont, n, len(arr))
    ke = irredundant(new)
    nw = reduce(ke)
    ne = irredundant(nw)
